...
# -*- coding=utf-8 -*-
"""
MultiGridDet postprocessing functions (standalone implementation).
Legacy postprocessing functions for MultiGridDet object detection.
"""
import numpy as np
import copy
from scipy.special import expit, softmax

# Note: We'll need to implement weighted_boxes_fusion locally or remove WBF dependency
# For now, we'll create a simple placeholder
def weighted_boxes_fusion(*args, **kwargs):
    """Placeholder for weighted boxes fusion - not used in current implementation"""
    return [], [], []

def denseyolo_decode(prediction, anchors, num_classes, input_dims, output_layer_id, scale_x_y=None, use_softmax=True, rescore_confidence = True):
    '''
        Decode final layer features to bounding box parameters.
        output_layer_id: 1 for 32 by 32 cell, 2 for 16x16 and 3 for 8x8 output heads
    '''

    batch_size = np.shape(prediction)[0]
    num_anchors = len(anchors)
    grid_size = np.array(prediction).shape[1:3]

    # Grid boxes
    # cell_x = np.reshape(np.tile(range(grid_size[0]), [grid_size[1]]), (1, grid_size[0], grid_size[1], 1))
    # cell_y = np.reshape(np.tile(range(grid_size[1]), [grid_size[0]]), (1, grid_size[0], grid_size[1], 1))
    # cell_y = np.transpose(cell_y, (0, 2, 1, 3))
    # cell_grid = np.tile(np.concatenate([cell_x, cell_y], -1), [batch_size, 1, 1, 1])
    ...
    # Scores
    objectness_probs = expit(prediction[..., 4])  # p_o (objectness score)
    objectness_probs = np.expand_dims(objectness_probs, axis=-1)
    anchor_probs = softmax(prediction[..., 5:5+num_anchors], axis=-1)
    class_probs = softmax(prediction[..., 5+num_anchors:], axis=-1)

    #raw box coordinate params
    
    raw_xy = (np.tanh(0.15*prediction[..., 0:2]) + expit(0.15*prediction[..., 0:2])) #np.clip(prediction[..., 0:2], -1.0, 2.0)
    raw_wh = prediction[..., 2:4]

    # raw box params converted to spacially rescaled box params using grid that predicted them
    # and the highest scoring anchor of the grid.
    # later this values will be scaled back to the actual input image size and then pass through NMS of some kind
    # before drawn back on to the image.
    box_xy = (raw_xy + cell_grid)
     
    # box_xy = np.maximum(box_xy, np.zeros_like(box_xy))
    # box_xy = np.minimum(box_xy, (np.ones_like(box_xy)* grid_size-1.0))

    predicted_anchor_index_per_grid = np.argmax(anchor_probs, axis=-1)
    anchors_per_grid = np.take(anchors, predicted_anchor_index_per_grid, axis=0)
    box_wh = anchors_per_grid * np.exp(raw_wh)

    box_xy /= grid_size #boxes independent of grid cell location as
    box_wh /= input_dims

    if rescore_confidence:
        class_probs = objectness_probs * np.expand_dims(np.max(anchor_probs, axis=-1), axis=-1) * class_probs
        
        # objectness_probs = np.max(class_probs, axis=-1)
        # objectness_probs = np.expand_dims(objectness_probs, axis=-1)
    #pos = np.where(objectness_probs >= 0.5)
    #np.set_printoptions(threshold=np.inf, precision=4, floatmode='fixed')
    #print(raw_xy[pos])
    prediction = np.concatenate([box_xy, box_wh, objectness_probs, class_probs], axis=-1)
    prediction = np.reshape(prediction, (batch_size, grid_size[0] * grid_size[1], num_classes + 5))
    return prediction


def denseyolo_correct_boxes(predictions, img_shape, model_image_size):
    ...
    box_xy *= image_wh
    box_wh *= image_wh
    prediction = np.concatenate([box_xy, box_wh, objectness_prob, class_probs], axis=-1)

    return prediction

def denseyolo_handle_predictions(predictions, image_shape,max_boxes=100, confidence=0.5, nms_threshold=0.15, use_iol=True, use_cluster_nms=False, use_wbf=False, rescore_confidence = True):
    boxes = predictions[..., 0:4]
    box_confidences = predictions[..., 4]
    box_class_probs = predictions[..., 5:]

    # if rescore_confidence:
    #     box_scores = box_class_probs
    ...
    #np.set_printoptions(threshold=20, precision=4, floatmode='fixed')
    
    
    if use_cluster_nms:
        # use Fast/Cluster NMS for boxes postprocess
        n_boxes, n_classes, n_scores = fast_cluster_nms_boxes(boxes, classes, scores, nms_threshold,use_iol=use_iol,
        ...
        n_boxes, n_classes, n_scores = nms_boxes(boxes, classes, scores, nms_threshold, use_iol=True, use_diou=False,
                                                 confidence=confidence, is_soft=False, use_exp=False)
    '''
    n_boxes = [np.array(boxes)]
    n_classes = [np.array(classes)]
...
def box_iou(boxes,use_iol=True):
    """
...
def box_diou(boxes,use_iol=True):
    """
    ...
    enclose_ymax = np.maximum(y[1:] + h[1:], y[0] + h[0])
...
def nms_boxes(boxes, classes, scores, iou_threshold, confidence=0.01, use_iol= True, use_diou=False, is_soft=False, use_exp=False, sigma=0.5):
    ...
    for c in set(classes):
        ...
        # print("before")
        # print(np.concatenate((b_nms, np.reshape(s_nms, (-1, 1))), axis=-1))
        while len(s_nms) > 0:
            ...
            #print(np.concatenate((b_nms, np.reshape(s_nms, (-1, 1))), axis=-1))
            if use_diou:
                iou = box_diou(b_nms, use_iol=use_iol)
                #iou = box_diou_matrix(b_nms, b_nms)[0][1:]
            else:
                iou = box_iou(b_nms, use_iol=use_iol)
...
def box_iou_matrix(boxes1, boxes2,use_iol=True):
    """
    ...
    if use_iol:
        iou = inter / np.maximum(area1[:, None], area2)  # iou = inter / (area1 + area2 - inter)
    else:
        iou = inter / (area1[:, None] + area2 - inter)  # iou = inter / (area1 + area2 - inter)
    return iou


def box_diou_matrix(boxes1, boxes2,use_iol=True):
    """
    ...
    Returns:
        diou (np.array[N, M]): the NxM matrix containing the pairwise
            IoU values for every element in boxes1 and boxes2
    """
    iou = box_iou_matrix(boxes1, boxes2,use_iol=use_iol)
...
def fast_cluster_nms_boxes(boxes, classes, scores, iou_threshold, confidence=0.1, use_cluster=True, use_iol = True, use_diou=True, use_weighted=True, use_matrix_nms=False, use_spm=False):
    """
    ...
    for c in set(classes):
        ...
        if use_diou:
            iou_matrix = box_diou_matrix(b_nms, b_nms,use_iol=use_iol)
        else:
            iou_matrix = box_iou_matrix(b_nms, b_nms,use_iol=use_iol)
...
def denseyolo_adjust_boxes(boxes, img_shape):
    '''
